...
export function BonCommandeCreateForm({ partners }: { partners: Partner[] }) {
  ...
  async function submit(formData: FormData) {
    if (!(await confirm({ title: "Créer ce bon de commande ?", description: "Un nouveau brouillon de bon de commande sera créé pour ce fournisseur.", confirmLabel: "Créer le bon" }))) return;
    setError(null);
  ...
  return (
    ...
    >
      ...
      <button className="min-h-11 rounded-full bg-ink-900 px-5 text-sm font-semibold text-white shadow-sm disabled:opacity-60" disabled={pending} type="submit">{pending ? "Création…" : "Créer le bon de commande"}</button>
    </form>
  );
}
