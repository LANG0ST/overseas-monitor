...
export function AvoirEditor({
  ...
  async function deleteDocument() {
    ...
    if (!(await confirm({ title: restoring ? "Restaurer cet avoir ?" : "Désactiver cet avoir ?", description: restoring ? "Il redeviendra disponible dans les avoirs actifs." : "Il restera disponible dans les documents inactifs et pourra être restauré.", confirmLabel: restoring ? "Restaurer" : "Désactiver", destructive: !restoring }))) return;
    startTransition(async () => {
      const result = restoring ? await restoreAvoirAction(document.id) : await deleteAvoirAction(document.id);
      if (applyResult(result)) router.push(restoring ? "/avoirs" : "/avoirs?inactive=1");
    });
  }

  function printInvoice() {
    printWithTitle(`Avoir-${document.number || `Brouillon-${document.client_name}`}`);
  }
      ...
      <div className="grid gap-4 xl:grid-cols-[260px_minmax(0,1fr)_340px]">
        ...
        <aside className="glass-card order-3 rounded-2xl p-5 print:hidden" ref={setLinePanelTarget}>
            ...
            <div className="mt-5 space-y-4 text-sm text-neutral-700">
              ...
              <p className="rounded-xl bg-primary-50 p-3 text-xs">
                {document.partenaire_id
                  ? "Client issu du répertoire : les coordonnées sont verrouillées."
                  : "Client saisi manuellement : les champs sont modifiables."}
              </p>
            </div>
          ) : null}
          {selectedTool === "totals" ? (
          ...
          {selectedTool === "delete" ? (
            <div className="mt-5 space-y-4">
              <p className="text-sm text-neutral-700">
                {document.is_active ? "L’avoir sera conservé dans les inactifs." : "L’avoir redeviendra disponible dans les actifs."}
              </p>
              <button
                ...
                type="button"
              >
                {document.is_active ? "Désactiver l’avoir" : "Restaurer l’avoir"}
              </button>
            </div>
          ) : null}
        </aside>
              ...
              <label className="block text-sm font-semibold text-neutral-900">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setDocument((current) => ({
                      ...current,
                      reference_facture_number: event.target.value,
                    }))
                  }
                  value={document.reference_facture_number ?? ""}
                />
              </label>
              <label className="block text-sm font-semibold text-neutral-900">
                Motif
                <input
                  ...
                  required
                  value={document.motif ?? ""}
                />
              </label>
              <fieldset className="space-y-2">
                <legend className="text-sm font-semibold text-neutral-900">
                  Mode de règlement de l’avoir
                </legend>
                {[
                  ["deduction", "Déduction sur facture à venir"],
                  ["virement", "Virement"],
                  ["cheque", "Chèque"],
                ].map(([value, label]) => (
                  <label className="flex items-center gap-2 text-sm text-neutral-800" key={value}>
                    <input
                      checked={document.avoir_payment_method === value}
                      disabled={locked}
                      onChange={() =>
                        setDocument((current) => ({
                          ...current,
                          avoir_payment_method:
                            current.avoir_payment_method === value ? null : value,
                        }))
                      }
                      type="checkbox"
                    />
              ...
              <label className="block text-sm font-semibold text-neutral-900">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setDocument((current) => ({
                      ...current,
                      avoir_payment_reference: event.target.value,
                    }))
                  }
                  placeholder="Référence"
                  value={document.avoir_payment_reference ?? ""}
                />
              </label>
            </div>,
            linePanelTarget,
