...
export function InvoiceLineRow({
  ...
  return (
    ...
    >
      ...
      <td className="break-words px-2 py-3 text-center text-neutral-700">{line.unit}</td>
      <td className="px-2 py-3 text-center text-neutral-900">{line.qty}</td>
      <td className="px-2 py-3 text-center text-neutral-900">
        {line.tva_rate ?? 20}%
      </td>
...
export function AvoirPage({
  ...
  return (
    ...
    >
      {showHeader ? (
        <header className="shrink-0 " data-invoice-header>
          <div className="grid grid-cols-[1.15fr_0.85fr] gap-8">
            ...
            <div className="pl-8 text-left">
              <p className="text-xl font-bold leading-tight">
                Avoir N° : {document.number || "Brouillon"}
              </p>
          ...
          <div className="mt-5 space-y-1  pt-4 text-center text-sm">
            <p className="font-bold">
              AVOIR SUR FACTURE N° {document.reference_facture_number || "—"}
            </p>
            <p className="text-red-500">
              <strong>Motif :</strong> {document.motif || "Non renseigné"}
            </p>
          </div>
        </header>
      ) : null}
      ...
      >
        <table className="w-full table-fixed text-sm">
          ...
          >
            <tr>
              <th className="w-[33%] px-3 py-3">Désignation à créditer</th>
              <th className="w-[8%] px-2 py-3 text-center">Unité</th>
              <th className="w-[9%] px-2 py-3 text-center">Nombre</th>
              <th className="w-[8%] px-2 py-3 text-center">TVA</th>
      ...
      {showTotals ? (
        <>
          ...
          <div className="mt-5  pt-3 text-[10px] leading-relaxed">
            <p className="font-semibold text-primary-900">
              Mode de règlement de l&apos;avoir
            </p>
            <p className="mt-1 text-sm ">
              {document.avoir_payment_method === "deduction" ? "☒" : "☐"}{" "}
              Déduction sur facture à venir &nbsp;&nbsp;{" "}
              {document.avoir_payment_method === "virement" ? "☒" : "☐"}{" "}
              Virement &nbsp;&nbsp;{" "}
              {document.avoir_payment_method === "cheque" ? "☒" : "☐"} Chèque
            </p>
            <p className="mt-1">
              Référence : {document.avoir_payment_reference || "—"}
            </p>
          </div>
          <div className="mt-1 grid grid-cols-2 gap-6 border-t border-primary-200 pt-3 text-left text-[10px] font-semibold text-primary-900">
            <div className="relative min-h-[90px]">
              CACHET ET SIGNATURE — OVERSEAS SERVICES
              {hasCachet ? (
                <Image
            ...
            <div className="min-h-[90px]">ACCUSÉ DE RÉCEPTION — CLIENT</div>
          </div>
          <footer
