...
export default async function BonCommandeDocumentPage({
  ...
  const [{ data: document, error: documentError }, { data: profile, error: profileError }, { data: engins, error: enginsError }] = await Promise.all([
    supabase
      .from("documents")
      .select("id, type, number, date, city, has_cachet, partenaire_id, client_name, client_ice, client_address, line_items, tva_rate, ht, tva, ttc, is_active, is_locked, validity_days, chantier, period_start, period_end, devis_fuel_driver, devis_payment_conditions, devis_bank_name, devis_iban")
      .eq("id", id)
      .maybeSingle(),
  ...
  return <BonCommandeEditor engins={engins ?? []} initialDocument={document} isAdmin={profile?.role === "admin"} />;
}
