...
export default async function AvoirDocumentPage({
  ...
  const [{ data: document, error: documentError }, { data: profile, error: profileError }, { data: engins, error: enginsError }] = await Promise.all([
    supabase
      .from("documents")
      .select("id, type, number, date, city, has_cachet, partenaire_id, client_name, client_ice, client_address, line_items, tva_rate, ht, tva, ttc, is_active, is_locked, motif, reference_facture_number, avoir_payment_method, avoir_payment_reference")
      .eq("id", id)
      .maybeSingle(),
