...
export default async function EnginsPage({
  searchParams,
}: {
  ...
  let query = supabase
    .from("engins")
    .select("id, name, unit, default_price, photo_url, note", {
      count: "exact",
    })
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-sm text-neutral-500">
            Matériel et services facturables
          </p>
          <h1 className="mt-1 text-3xl font-semibold">Parc d’engins</h1>
        </div>
        <div className="flex gap-2">
          <Link
            className="rounded-full border bg-white px-4 py-2 text-sm text-ink-900 shadow-sm"
            href={showInactive ? "/engins" : "/engins?inactive=1"}
          >
      ...
      <form
        className="glass-card flex flex-wrap items-end gap-3 rounded-2xl p-4"
        method="get"
      >
        {showInactive ? (
            ...
            <div className="flex h-36 items-center justify-center bg-primary-50">
              {engin.imageUrl ? (
                ...
                <span className="text-3xl font-semibold text-primary-300">
                  {engin.name.slice(0, 1).toUpperCase()}
                </span>
              )}
            </div>
            <div className="space-y-3 p-5">
              <div>
                <h2 className="font-semibold">{engin.name}</h2>
                <p className="mt-1 text-sm text-neutral-500">
                  {engin.unit} · {Number(engin.default_price).toFixed(2)} MAD
                </p>
              </div>
              <p className="line-clamp-2 text-sm text-neutral-600">
                {engin.note || "Aucune note"}
              </p>
              {editable ? (
                <div className="flex gap-2">
                  <Link
                    ...
                    href={`/engins/${engin.id}/edit`}
                  >
                    Modifier
                  </Link>
                  <ConfirmedForm
                    action={setEnginActive.bind(
                      null,
                      engin.id,
                      showInactive,
                      showInactive ? "/engins?inactive=1" : "/engins",
                    )}
                    confirmationTitle={showInactive ? "Restaurer cet engin ?" : "Désactiver cet engin ?"}
                    confirmationDescription={showInactive ? "L’engin redeviendra disponible dans les sélections." : "L’engin sera conservé dans les éléments inactifs."}
      ...
      {engins.length === 0 ? (
        <p className="rounded-2xl border border-dashed p-8 text-center text-sm text-neutral-500">
          Aucun engin dans cette vue.
        </p>
      ) : null}
      {totalPages > 1 ? (
        <nav
          aria-label="Pagination du parc d’engins"
          className="flex items-center justify-center gap-3"
        >
          <Link
            aria-disabled={page === 1}
            className="rounded-full border bg-white px-4 py-2 text-sm text-ink-900 shadow-sm aria-disabled:pointer-events-none aria-disabled:opacity-40"
          ...
          <Link
            ...
            className="rounded-full border bg-white px-4 py-2 text-sm text-ink-900 shadow-sm aria-disabled:pointer-events-none aria-disabled:opacity-40"
            href={queryString(Math.min(totalPages, page + 1))}
          >
            Suivant
          </Link>
        </nav>
